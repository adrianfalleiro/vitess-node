...
export class Decoder {
  static decodeValue(buf: Buffer, type: TypeJson): string | boolean | Date | Buffer | number | bigint | null | object {
    switch (type) {
      // Integer types
      case "INT64":
      case "UINT64":
  ...
  static decodeRow(row: RowJson, fields: FieldJson[]): Record<string, any> {
    ...
    for (let i = 0; i < fields.length; i++) {
      ...
      obj[field.name] = this.decodeValue(slice, field.type);
      offset += len;
    }
